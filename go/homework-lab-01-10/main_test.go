...
func TestMergeSlices1(t *testing.T) {

    expected := []int{1, 2, 3, 4, 5}

    result := MergeSlices1([]float32{1.1, 2.1, 3.1}, []int32{4, 5})
    if !reflect.DeepEqual(result, expected) {
        t.Error("expected", expected, "have", result)
    }
}

func TestMergeSlices2(t *testing.T) {

    expected := []int32{1, 2, 3, 4, 5}

    result := MergeSlices2([]float32{1.1, 2.1, 3.1}, []int32{4, 5})
    if !reflect.DeepEqual(result, expected) {
        t.Error("expected", expected, "have", result)
    }
}
